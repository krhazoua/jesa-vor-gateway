...
// `state` carries the backend redirect URI (used at token exchange), the CSRF
// nonce, and the validated frontend return URI. Defined here so the client
// encoder and server decoder never drift.
export type OAuthState = { redirectUri: string; nonce?: string; returnUri?: string };
