...
export async function getAnalyticsSeries() { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const transitions = await db.select({ fromStatus: requestHistory.fromStatus, toStatus: requestHistory.toStatus, createdAt: requestHistory.createdAt }).from(requestHistory).orderBy(desc(requestHistory.createdAt)).limit(5000); const approvalRows = await db.select({ createdAt: approvals.createdAt, decidedAt: approvals.decidedAt, decision: approvals.decision }).from(approvals).orderBy(desc(approvals.createdAt)).limit(5000); return { transitions: aggregateTransitions(transitions), approvalTimes: aggregateApprovalTimes(approvalRows) }; }
