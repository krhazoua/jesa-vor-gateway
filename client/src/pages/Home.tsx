...
const nav = [
  { label: "Operations", icon: Activity, active: true }, { label: "Requests", icon: FileText },
  { label: "Approvals", icon: ClipboardCheck, badge: "01" }, { label: "Validation", icon: ShieldCheck },
  { label: "Audit trail", icon: Database }, { label: "System health", icon: Radar },
];
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="nav-list">{nav.map(item => <button key={item.label} className={`nav-item ${item.active ? "active" : ""}`} onClick={() => item.label !== "Operations" && toast(`${item.label} view is available in the full gateway build.`)}><item.icon size={16} /><span>{item.label}</span>{item.badge && <b>{item.badge}</b>}</button>)}</nav>
        <div className="sidebar-foot"><div className="zone-card"><div className="eyebrow">SECURITY ZONES</div><div className="zone-row"><span className="zone-dot blue" />MODULE 3 · psM+O <Check size={13} /></div><div className="zone-row"><span className="zone-dot amber" />MODULE 2 · DMZ <Check size={13} /></div><div className="zone-row"><span className="zone-dot green" />MODULE 1 · CPC <Check size={13} /></div></div><div className="user-row"><div className="avatar">AK</div><div><strong>A. Kharbouch</strong><small>Process Control Engineer</small></div><ChevronRight size={15} /></div></div>
      </aside>
