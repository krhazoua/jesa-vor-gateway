      ...
      <main className={`main-content ${paused ? "simulation-paused" : ""}`}>
        ...
        <div className="content-wrap">
          ...
          <section className="workbench">
            <div className="requests-panel panel"><div className="panel-head"><div><h2>Recent requests</h2><span className="panel-caption">APC → DMZ → CPC / newest first</span></div><div className="panel-tools"><div className="search"><Search size={14} /><input placeholder="Filter requests" value={query} onChange={e => setQuery(e.target.value)} /></div><button className="filter-btn" onClick={() => toast(query ? `Request filter active: ${query}` : "Enter a request ID, tag, equipment, or source to filter the monitor.")}><SlidersHorizontal size={14} /> Filter</button></div></div><div className="request-table sim-request-stream"><div className="table-head"><span className="stream-scanline" /><span>REQUEST / TIME</span><span>SOURCE</span><span>VARIABLE / TAG</span><span>Δ REQUEST</span><span>DECISION</span></div>{filtered.map(r => <button className={`request-row ${selected.id === r.id ? "selected" : ""}`} key={r.id} onClick={() => selectRequest(r)}><div><strong className="mono">{r.id}</strong><small>{r.time} UTC</small></div><div className="mono source">{r.source}</div><div><strong>{r.variable}</strong><small className="mono">{r.tag} · {r.uc}</small></div><div className={`mono delta ${r.delta.startsWith("+") ? "positive" : ""}`}>{r.delta} {r.unit}</div><Status value={r.status} /></button>)}</div><div className="panel-foot"><span>Showing {filtered.length} of {operationalStats.total} records · {operationalSource}</span><button onClick={() => toast("Request archive is ready in the full gateway build.")}>VIEW REQUEST ARCHIVE <ChevronRight size={13} /></button></div></div>
