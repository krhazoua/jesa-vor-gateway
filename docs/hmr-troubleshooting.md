...
`setupVite` now receives the selected application port and configures `server.hmr.clientPort` with a validated value. The server selects its port before mounting Vite, then passes that exact port into the Vite middleware setup. In the managed WebDev preview, where the reverse proxy can close upgraded WebSocket connections during process restarts, HMR is disabled and the managed HTML handler runs before Vite's HTML fallback so the transformed document is cleaned before it reaches the browser. As a second defense for a browser holding an older page, managed requests to `/@vite/client` and `/@react-refresh` are intercepted with a compatibility module that preserves CSS injection but exposes no WebSocket transport. Development HTML is served with `Cache-Control: no-store, no-cache, must-revalidate`; Vite remains available for source modules and assets. WebDev's managed reload strategy remains active. Ordinary local development retains HMR with the application port. Invalid values safely fall back to port `3000`; no production API URL, authentication behavior, or backend boundary was changed.
