...
## `/compliance` Missing result repair — 2026-08-25

The reported `Missing result` and `Gateway connection could not be established` messages were traced to the client transport boundary rather than an undefined `configuration.compliance` server procedure. The protected procedure returns `{ generatedAt, evidence }`, and a new protected-caller regression test confirms a defined result with six evidence rows. The failure mode was an incomplete or transient malformed JSON tRPC batch envelope reaching the tRPC transformer, where it surfaced as an opaque parser error; the retry fallback could separately surface the gateway-connection message.

The shared read-only tRPC transport guard now validates JSON envelopes before handing them to the transformer, retries incomplete read-only responses using the existing bounded retry policy, and returns a structured 502 incomplete-response error after the retry budget. This prevents `Missing result` from escaping as an unhandled parse failure while preserving normal tRPC result/error envelopes and the no-retry rule for mutations. Focused coverage passes with 9 tests, including the malformed `[{}]` batch regression and protected compliance caller test. Full verification passes with 134 Vitest tests, lint, TypeScript, production build, bundle budgets, and `git diff --check`. The direct unauthenticated compliance probe correctly returns structured 401. Live authenticated browser confirmation requires a valid session in the browser or secure E2E storage state; no credentials were fabricated.

## Authenticated E2E artifact retention — 2026-08-25

The authenticated-browser CI job now persists each CSV/XLSX download through Playwright’s per-test output directory and always uploads `playwright-report/` and `test-results/` as the `authenticated-browser-evidence` artifact with a 14-day retention window. This captures HTML reports, failure screenshots, retained traces/videos, and downloaded report files even when a browser test fails. The secure-state gate remains `secrets.E2E_STORAGE_STATE != ''`, and the temporary JSON storage file is removed after artifact upload.

The sandbox has no secure `E2E_STORAGE_STATE`, so the authenticated browser tests continue to skip by design rather than using fabricated credentials. The managed preview’s session-aware route renderer has been verified through the authenticated preview capture, while an independent browser session without stored credentials correctly remains at the secure login boundary.
