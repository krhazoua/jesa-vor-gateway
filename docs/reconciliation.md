...
The supplied specification requested a dedicated NE178 Compliance surface. The application now exposes `/compliance` behind the protected route gate and navigation. Its six-row matrix is populated by the protected `configuration.compliance` evidence contract, which records the source procedure, evidence statement, status, and integration-gate state for Authentication & Authorization, Verification, Mapping, Propagation, Acceptance, and Mapping Verification. Propagation and Mapping Verification are visibly marked as gated because the read-only edge adapter has no configured plant write path.
